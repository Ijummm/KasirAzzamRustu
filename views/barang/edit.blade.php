...
@section('content')
    <form action="{{ route('barang.update', $barangs->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="nama" value="{{ $barangs->nama }}" class="mt-1 w-full border border-green-300 rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700">Stok</label>
            <input type="number" name="stok" value="{{ $barangs->stok }}" class="mt-1 w-full border border-green-300 rounded px-3 py-2" required>
        </div>
